...
const TotalGeneratedPolicy = () => {
  const [exportData, setExportData] = useState()
  console.log('ExportData', exportData)

  // const headers = [
  //   { label: 'Issuance Date', key: 'details.firstName' },
  //   { label: 'Registration Number', key: 'details.lastName' },
  //   { label: 'Customer Name', key: 'job' },
  //   { label: 'Insurer', key: 'job' },
  //   { label: 'Vehicle Type', key: 'job' },
  //   { label: 'Case Type', key: 'job' },
  //   { label: 'Policy Type', key: 'job' },
  //   { label: 'Case Mode', key: 'job' },
  //   { label: 'Policy Number', key: 'job' },
  ...
  useEffect(() => {
    ...
    GetData('admin/generated_policy').then(response => {
      if (
        ...
        setAllPolicies(transformedData)
        setFilteredData(transformedData.reverse())
        ...
        <div class=''>
          ...
          >
            <div className='border border-info rounded-4  p-2 container-fluid'>
              ...
              {/* <Row><Col><CSVLink headers={headers} data={exportData}  >Export</CSVLink></Col></Row> */}
                  ...
                  <tbody style={{ fontSize: '14px' }}>
                    {filteredData &&
                      filteredData.map((info, index) => (
                        <tr key={index}>
                          <td>{info?.lead_id || 'N/A'}</td>

                          <td>{info?.lead_data?.caseType}</td>

                          <td>
                            <div>{info?.policyType}</div>
                          </td>
                          <td>
                            <div>{info?.policyNumber}</div>
                          </td>
                          ...
                          <td>{info?.lead_data?.currentInsuerName}</td>
                          <td>{info?.lead_data?.insurerName || 'N/A'}</td>
                          <td>{info?.lead_data?.net_premium}</td>
                          <td>{info?.lead_data?.gst_Tax_amount}</td>
                          <td>{info?.lead_data?.final_premium}</td>
                          <td>
                            <span
...
export default TotalGeneratedPolicy
